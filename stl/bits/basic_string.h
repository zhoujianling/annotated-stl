...
_GLIBCXX_BEGIN_NAMESPACE_CXX11
  ...
  template<typename _CharT, typename _Traits, typename _Alloc>
	...
	pointer _M_p; // The actual data.
      ...
      union
      {
      // _M_local_buf 的大小恒为 16 字节
	_CharT           _M_local_buf[_S_local_capacity + 1];
      // 8 字节表示已分配空间容量，如果字符串太短，则此字段无意义
	size_type        _M_allocated_capacity;
      ...
      // 私有函数，获取局部存储的指针(当字符串很短的时候，不保存在堆里，而是在对象内部)
      pointer
      _M_local_data()
      {
#if __cplusplus >= 201103L
	...
	return const_pointer(_M_local_buf);
      ...
      // 私有函数，设置对象内部保存的已分配空间的大小的字段
      void
      _M_capacity(size_type __capacity)
      { _M_allocated_capacity = __capacity; }

      // 私有函数，设置对象内部保存的字符串长度的大小的字段
      void
	...
	traits_type::assign(_M_data()[__n], _CharT());
      ...
      // 当前字符串是否保存在对象内部（栈上面）
      // 就是看 _M_p 是不是指向 _M_local_buf 上面
      bool
      _M_is_local() const
	...
	{ return this->append(__svt); }
       ...
       *  向当前字符串的尾部拼接字符串
       *  @brief  Append a string to this string.
       *  @param __str  The string to append. *  @return  Reference to this string.
       */
      basic_string&
	...
	_M_replace_dispatch(const_iterator __i1, const_iterator __i2,
      ...
      // 向当前字符串的尾部拼接字符串，具体实现在 basic_string.tcc 中
      basic_string&
...
_GLIBCXX_END_NAMESPACE_CXX11
  ...
  */
  ...
  // 默认情况下不会使用这个 basic_string 的实现，请看上面的版本, 大概在 80-100行左右声明
  template<typename _CharT, typename _Traits, typename _Alloc>
    class basic_string
    {
          // ...
